...
# Override I/O: CATH_EDC_DIR (default ../edc), CATH_SDTM_OUT (default ./sdtm-derived)
...
EDC_DIR <- Sys.getenv("CATH_EDC_DIR", unset = file.path(ENV_DIR, "edc"))
OUT     <- Sys.getenv("CATH_SDTM_OUT", unset = file.path(SCRIPT_DIR, "sdtm-derived"))

source(file.path(SCRIPT_DIR, "common.R"))

REG <- list(
  dm     = list(file = "derive_dm.R",     fn = "derive_dm"),
  suppdm = list(file = "derive_suppdm.R", fn = "derive_suppdm"),
  vs     = list(file = "derive_vs.R",     fn = "derive_vs"),
  # IE (inclusion/exclusion) is intentionally omitted: SDTM IE holds only criteria
  # NOT met (eligibility exceptions); every CATH subject met all criteria, so IE
  # would be empty. The criterion definitions belong in trial-design TI (not in
  # scope here). The collected Y/N responses live in the EDC IE form.
  ...
  suppex = list(file = "derive_suppex.R", fn = "derive_suppex"),
  ae     = list(file = "derive_ae.R",     fn = "derive_ae"),
  lb     = list(file = "derive_lb.R",     fn = "derive_lb"),
  mb     = list(file = "derive_mb.R",     fn = "derive_mb"),
  rs     = list(file = "derive_rs.R",     fn = "derive_rs"),
  rp     = list(file = "derive_rp.R",     fn = "derive_rp"),
...
# Source every derivation up front (SUPP-- programs call their parent derive_*).
