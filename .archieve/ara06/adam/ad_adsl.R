...
# Input: dm, ex, ds
#
# ARA06 (RA, adalimumab vs etanercept): treatment from EX, disposition from DS,
# population flags, demographic groupings.
library(admiral)
library(dplyr)
library(lubridate)
library(stringr)
...
adsl <- dm %>%
  ...
  derive_var_trtdurd() %>%
  # disposition: end-of-study status / date / reason
  derive_vars_merged(
