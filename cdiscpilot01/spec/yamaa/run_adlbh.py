...
def main() -> None:
    out = run_yamaa("adlbh-prevmap.yaml")
    ...
    staged = work / "lb_h_with_prev.parquet"
    joined.write_parquet(staged)
    print(f"  -> work/lb_h_with_prev.parquet: {joined.shape[0]} x {joined.shape[1]}",
          flush=True)

    run_yamaa("adlbh.yaml")
