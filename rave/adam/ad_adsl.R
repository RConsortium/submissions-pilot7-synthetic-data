...
# Input: dm, ex, ds
#
# RAVE (rituximab vs cyclophosphamide, ANCA vasculitis): treatment from EX,
# disposition + death from DS/DM, population flags, demographic groupings.
library(admiral)
library(dplyr)
library(lubridate)
library(stringr)
...
adsl <- dm %>%
  ...
  # death date from DM.DTHDTC (DTHFL already on DM)
