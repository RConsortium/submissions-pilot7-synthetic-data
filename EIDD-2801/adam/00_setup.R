# Local-data setup for the admiral ADaM derivations (EIDD-2801-1001-UK).
...
# environment's generated SDTM (../sdtm/sdtm-derived/xpt/, produced by
# sdtm/run_all.R + sdtm/export_conformant.py). Mirrors the cdiscpilot1 adam setup.
#
# Paths are cwd-relative; run from adam/ (see GUIDE.md). Override via env vars:
#   ADAM_SDTM_DIR (XPT source)   ADAM_OUT_DIR (output dir).
...
# read_xpt restores Char/Num types + labels; re-coerce *DTC back to ISO-8601
# character (admiral's derive_vars_dt*() expect --DTC text).
...
.adam_var_labels <- c(
  ...
  TRTSDT = "Datetime of First Exposure to Treatment",
  TRTEDT = "Datetime of Last Exposure to Treatment",
  ...
  ATPT = "Analysis Timepoint", VISIT = "Visit Name", VISITNUM = "Visit Number",
...
save_adam <- function(df, name) {
  # Drop SDTM-only variables admiral pulls in via full-domain merges.
