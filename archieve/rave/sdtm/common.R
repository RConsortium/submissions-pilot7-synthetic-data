# common.R — shared helpers for the per-domain CRF -> SDTM derivation programs (RAVE).
#
...
read_form <- function(name) read_csv_chr(file.path(FORMS, paste0("RAVE_CRF_", name, ".csv")))
...
epoch_of <- function(visit) ifelse(visit %in% c("SCREENING", ""), "SCREENING",
                            ifelse(visit == "FOLLOW-UP", "FOLLOW-UP", "TREATMENT"))

# Resolve VISIT name / VISITNUM / day OFFSET for a frame carrying a CRF visit-code
# column. Adds VISIT, VISITNUM, .OFFSET columns.
